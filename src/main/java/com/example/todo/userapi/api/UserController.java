    ...
    public ResponseEntity<?> loadFile(
        ...
        try {
            ...
            if (!profileFile.exists()) {
                return ResponseEntity.notFound().build(); //프로필 파일이 없다면 404 보내줌
            }
...
//            headers.set("Content-type", "image/jpeg"); 가능
//            headers.setContentType(MediaType.IMAGE_JPEG); 가능
            MediaType contentType = findExtentionAndGetMediaType(filePath);
            if (contentType == null) {
    ...
    private MediaType findExtentionAndGetMediaType(String filePath) {

        // 파일 경로에서 확장자 추출하기
        // C:/todo_upload/asdfsadfsaaafdsa_jisu.jpg
    ...
    // S3에서 불러온 프로필 사진 처리
    @GetMapping("/load-s3")
    public ResponseEntity<?> loadS3( //토큰 처리해주는 아노테이션
            @AuthenticationPrincipal TokenUserInfo userInfo
    ) {
